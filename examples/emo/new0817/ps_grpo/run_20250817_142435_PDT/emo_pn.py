...
# One lenient "format" pattern (structure + allowed labels), same style:
FORMAT_RE = re.compile(
    r".*<tone_thought>.*</tone_thought>.*"
    r"<emotion_thought>.*</emotion_thought>.*"
    r"<tone>\s*(positive|negative)\s*</tone>.*"
    r"<answer>\s*(anger|disgust|fear|joy|sadness|surprise)\s*</answer>.*",
...
# Extractors at the same leniency level (simple search with DOTALL/IGNORECASE):
TONE_RE = re.compile(r"<tone>\s*(positive|negative)\s*</tone>", re.DOTALL | re.IGNORECASE)
EMO_RE  = re.compile(r"<answer>\s*(anger|disgust|fear|joy|sadness|surprise)\s*</answer>", re.DOTALL | re.IGNORECASE)
