...
EXPERIMENT_NAME=qwen2_5_vl_3b_emoset_${ROLLOUT_NUMBER}_grpo_emo_think_new_nocoherence
...
python3 -m verl.trainer.main \
    ...
    data.format_prompt=./examples/format_prompt/new/emothink.jinja \
    worker.actor.model.model_path=${MODEL_PATH} \
    worker.rollout.tensor_parallel_size=1 \
    worker.rollout.n=${ROLLOUT_NUMBER} \
    worker.rollout.logprobs=null \
    worker.reward.reward_function=./examples/reward_function/new/emothink.py:compute_score \
